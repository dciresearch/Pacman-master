...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining = 0, catchExceptions=False, timeout=30 ):
    ...
    from multiAgents import backup
    ...
    commonModel = None
    if hasattr(pacman, 'model'):
        commonModel=pacman.model

    numGames = numTraining+numGames

    for i in range( numGames ):
        iter_layout = layout[i%len(layout)]
        ...
        if beQuiet:
            ...
            gameDisplay = display
            rules.quiet = False
        game = rules.newGame( iter_layout, pacman, ghosts, gameDisplay, beQuiet, catchExceptions)
        game.run()
        if not beQuiet: 
            games.append(game)
        if commonModel is not None:
            backup(commonModel, game)

        if record:
            import time, pickle
            fname = ('recorded-game-%d' % (i + 1)) +  '-'.join([str(t) for t in time.localtime()[1:6]])
            f = open(fname, 'w')
            components = {'layout': iter_layout, 'actions': game.moveHistory}
